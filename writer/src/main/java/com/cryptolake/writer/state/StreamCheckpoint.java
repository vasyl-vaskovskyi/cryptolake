...
/**
 * Durable checkpoint for a stream: the last-written envelope metadata so writer can resume from the
 * correct Kafka offset on restart (Tier 1 §6 — replay over reconstruction).
 *
 * <p>Ports Python's {@code StreamCheckpoint} (design §6.6). PG table {@code stream_checkpoint}, PK
 * {@code (exchange, symbol, stream)}.
 *
