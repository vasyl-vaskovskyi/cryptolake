...
a. **WebSocket connection.** In steady state the Collector maintains a single consolidated WebSocket connection to Binance USD-M Futures at `wss://fstream.binance.com/stream`; during a planned WS rotation overlap (§8.b) up to two connections run in parallel briefly. Note: this is the `/stream` endpoint, not `/ws` — the `/stream` endpoint guarantees every event is wrapped in `{"stream": "<name>", "data": {...}}` regardless of whether one or many streams are subscribed, while `/ws` + SUBSCRIBE returns bare events with no wrapper. The wrapper is the routing key for multi-stream layouts (§8.c) so the consolidated endpoint is mandatory.

   Streams are subscribed via a `{"method":"SUBSCRIBE","params":[...],"id":<int>}` message after connect, not encoded into the URL (the URL form gets unwieldy with this many streams and is harder to mock for tests). The Collector **blocks on the SUBSCRIBE acknowledgement** — Binance replies with `{"result":null,"id":<int>}` matching the request `id` — before treating the subscription as active. A SUBSCRIBE that times out (no ACK within 10 seconds) or returns an error response is treated as a connection failure: close, reconnect, retry subscription. Without this gate, a silently-dropped subscription would look identical to a quiet stream.
