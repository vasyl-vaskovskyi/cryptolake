...
docker compose exec writer dd if=/dev/zero of="${HOST_DATA_DIR:-/data}/fill_disk.tmp" bs=1M count=500 2>/dev/null || true
echo "2. Checking disk usage..."
docker compose exec writer df -h "${HOST_DATA_DIR:-/data}"
...
echo "5. Cleaning up..."
docker compose exec writer rm -f "${HOST_DATA_DIR:-/data}/fill_disk.tmp"
